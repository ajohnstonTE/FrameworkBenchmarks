...
RUN curl -s --compressed -o /etc/apt/trusted.gpg.d/keydb.gpg https://download.keydb.dev/keydb-ppa/keydb.gpg
RUN curl -s --compressed -o /etc/apt/sources.list.d/keydb.list https://download.keydb.dev/keydb-ppa/keydb.list
RUN apt update
RUN apt install -y keydb-tools

ENV REDIS_CLIENT=REDISSON

CMD ["sh", "-c", "keydb-server --protected-mode no --port 6379 --server-threads 7 --server-thread-affinity true --save </dev/null &>/dev/null & java -jar lib/resin.jar console"]
